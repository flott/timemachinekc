<script>
  ...
  $: if (map && $activeAerial && $activeAerial !== currentAerial) {
    ...
    if ($activeAerial.attribution === "Nearmap") {
      activeTiles = L.tileLayer($activeAerial.url, {
        ...
        maxZoom:
          $activeAerial.maxzoom + 1 <= 22 ? $activeAerial.maxzoom + 1 : 22,
        maxNativeZoom: $activeAerial.maxzoom,
        minNativeZoom: $activeAerial.minzoom,
        minZoom: 9,
      }).addTo(map);
    } else {
      activeTiles = tiledMapLayer({
        url: $activeAerial.url,
        attribution: $activeAerial.attribution,
        maxZoom:
          $activeAerial.maxzoom + 1 <= 22 ? $activeAerial.maxzoom + 1 : 22,
        maxNativeZoom: $activeAerial.maxzoom,
  ...
  $: if ($location.label && map) {
    ...
    map.flyTo(
      $location.latlng,
      $activeAerial.maxzoom < 20 ? $activeAerial.maxzoom : 20
    );
  ...
  $: if (map && $portalDate && magnifyingGlass) {
    if ($activePortal.attribution === "Nearmap") {
